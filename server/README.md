...
| `GOOGLE_CLIENT_ID` | Google OAuth 2.0 Web Client ID used by GIS and server-side ID-token audience verification. No client secret or redirect URI is required. |
| `STORAGE_DRIVER` | `local` (disk, dev) or `s3` (S3-compatible). |

> **Credentials to provide:** the model API key (OpenAI), and — only if you want
> those features — SMTP creds (email reset) and a Google OAuth Web Client ID.
...
| GET | `/auth/google/config` | | public GIS configuration |
| POST | `/auth/google` | `{credential}` | verifies a GIS ID token, links/creates user, issues normal session |
