...
function cleanPrompt(value) {
  return String(value || '')
    ...
    .replace(/\s*```$/i, '')
    .replace(/^\s*(?:optimized|customized|rewritten|final)\s+prompt\s*:\s*/i, '')
    .trim();
}
...
export async function customizePrompt({ originalPrompt, changeRequest, additionalInstructions = '' }) {
  const json = await callJson({
    system: customizeSystem(),
    user: customizeUser({ originalPrompt, changeRequest, additionalInstructions }),
  });
  const prompt = cleanPrompt(json?.prompt);
  if (!prompt) throw serverError('AI customization is temporarily unavailable. Please try again.');
  return { prompt, source: 'model' };
}

export async function optimizePrompt({ originalPrompt, platform, outputType, optimizationLevel }) {
  const json = await callJson({
    system: optimizeSystem({ platform, outputType, optimizationLevel }),
    user: optimizeUser(originalPrompt),
  });
  const prompt = cleanPrompt(json?.prompt);
  if (!prompt) throw serverError('AI optimization is temporarily unavailable. Please try again.');
  return { prompt, source: 'model' };
}
