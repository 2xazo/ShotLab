...
(function () {
  ...
  const SL = {
    ...
    customizePrompt: (payload) => req('POST', '/ai/customize', payload),
    optimizePrompt: (payload) => req('POST', '/ai/optimize-platform', payload),
    // ---- uploads ----
    upload: (file) => {
    ...
    promptShowcase: () => req('GET', '/api/prompt-showcase'),
    assetUrl: (assetPath) => {
      if (!assetPath) return '';
      return /^https?:\/\//i.test(assetPath) ? assetPath : BASE + (assetPath.startsWith('/') ? assetPath : '/' + assetPath);
    },
  };
